    ...
    async def init_pool(self):
        ...
        if not self.pool:
            self.pool = await aiomysql.create_pool(
                ...
                db=self.db_config.get("database", "douyin_cookies"),
    ...
    async def save_cookie(self, service: str, cookie: str):
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                # 插入新的 Cookie
                await cursor.execute(
                    'INSERT INTO cookies (service, cookie, updated_at, is_active) VALUES (%s, %s, %s, 1)',
...
class DouyinWebCrawler:
    ...
    async def update_cookie(self, cookie: str):
        ...
        # 保存到数据库
        await db_manager.save_cookie(service, cookie)
